...
import { ApiResponse, Subject, Topic, SubTopic, Test, Question } from '../types';

// Auth
export const login = (userId: string, password: string) =>
  api.post<ApiResponse<{ token: string; user: any }>>('/auth/login', { userId, password });
...
export const publishTest = (id: string) =>
  api.put<ApiResponse<Test>>(`/tests/${id}`, { status: 'live' });
